...
var FileManager = (function () {
     ...
     */
    FileManager.compileBinaries_ = function (osType) {
        var binaries = {};
        if (FileManager.checkOS_(osType, binaries_1.StandAlone)) {
            binaries[binaries_1.StandAlone.id] = new binaries_1.StandAlone();
        }
        if (FileManager.checkOS_(osType, binaries_1.ChromeDriver)) {
            binaries[binaries_1.ChromeDriver.id] = new binaries_1.ChromeDriver();
        }
        if (FileManager.checkOS_(osType, binaries_1.IEDriver)) {
            binaries[binaries_1.IEDriver.id] = new binaries_1.IEDriver();
        }
        if (FileManager.checkOS_(osType, binaries_1.AndroidSDK)) {
            binaries[binaries_1.AndroidSDK.id] = new binaries_1.AndroidSDK();
        }
        if (FileManager.checkOS_(osType, binaries_1.Appium)) {
            binaries[binaries_1.Appium.id] = new binaries_1.Appium();
        }
        return binaries;
    };
    /**
     ...
     */
    FileManager.setupBinaries = function () { return FileManager.compileBinaries_(os.type()); };
    /**
     ...
     */
    FileManager.getExistngFiles = function (outputDir) {
        try {
    ...
    FileManager.downloadedBinaries = function (outputDir) {
        ...
        var existingFiles = FileManager.getExistngFiles(outputDir);
        var downloaded = {};
     ...
     */
    FileManager.toDownload = function (binary, outputDir) {
        var osType = os.type();
        var osArch = os.arch();
        var filePath;
        var readData;
        ...
        if (downloaded[binary.id()]) {
            ...
            for (var index in versions) {
                ...
                if (v === version) {
                    ...
                    var contentLength = downloader_1.Downloader.httpHeadContentLength(binary.url(osType, osArch));
