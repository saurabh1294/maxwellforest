...
function downloadAndroidUpdates(sdkPath, targets, search_all, auto_accept) {
    return runAndroidSDKCommand(sdkPath, 'update', ['sdk', '-u'].concat(search_all
        ? ['-a'] : []).concat(['-t', targets.join(',')]), { stdio: auto_accept ? 'pipe' : 'inherit' }, auto_accept ? respondFactory('Do you accept the license', 'y') : noop);
}
...
function setupHardwareAcceleration(sdkPath) {
    ...
    if (os.type() == 'Darwin') {
        ...
        child_process.spawnSync('runas', ['/noprofile', '/user:Administrator',
            path.join(sdkPath, 'extras', 'intel', 'Hardware_Accelerated_Execution_Manager', 'silent_install.bat')], { stdio: 'inherit' });
    }
}
// Get a list of all the SDK download targets for a given set of APIs and ABIs
function getAndroidSDKTargets(apiLevels, abis) {
    return apiLevels.map(function (level) {
...
function sequentialForEach(array, func) {
    ...
    array.forEach(function (x) {
        ret = ret.then(function () { return func(x); });
    });
    return ret;
}
...
function makeAVD(sdkPath, desc, version) {
    return runAndroidSDKCommand(sdkPath, 'delete', ['avd', '--name', desc.avdName(version)], {})
        ...
        return runAndroidSDKCommand(sdkPath, 'create', ['avd', '--name', desc.avdName(version), '--target', desc.api, '--abi',
            desc.abi], { stdio: 'pipe' }, respondFactory('Do you wish to create a custom hardware profile', 'no'));
    });
}
...
function android(sdkPath, apiLevels, abis, acceptLicenses, version, logger) {
    ...
    downloadAndroidUpdates(sdkPath, tools, false, acceptLicenses).then(function () {
